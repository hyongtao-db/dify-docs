...
2. 「送信」ボタンをクリックすると、システムがJSONデータを自動的に解析し、JSON Schemaに変換します。

![]()

**AIでJSON Schemaを生成する**

1. 「AI生成」アイコンをクリックし、モデル（GPT-4など）を選択します。入力ボックスにJSON Schemaの説明を入力します。例：

> 「ユーザー名、年齢、および趣味を含むJSON Schemaが必要です。」

2. 「生成」をクリックすると、システムが自動的にJSON Schemaを生成します。
...
- 既存のJSON Schema（または API レスポンス例）を直接貼り付けて手動で調整したい場合。
...
3. 「保存」をクリックすると、システムがJSON Schemaを自動的に検証し、保存します。

**フィールドの編集・削除**

JSONコードボックスで、フィールドのタイプ、説明、デフォルト値などの項目を直接編集・削除し、「保存」をクリックします。
...
**シナリオ**: Dify Workflowを使用してユーザーフィードバック分析ワークフローを構築する際に、LLMノードはユーザー評価を読み取り、標準化された評価とコメントを返します。これにより、後続ノードがデータを正しく処理できるよう、データ形式の一貫性を保ちます。

**解決策**: ワークフローのLLMノードで、**JSON Schemaエディタ**を使用してLLM出力の構造化フォーマットを定義できます。これにより、LLM は自由形式のテキストではなく、定義済みのフォーマットに従って結果を出力します。

**操作手順**:

1. **LLM**ノードの**JSON Schema エディタ**で、以下のフィールドを追加します。これによりLLMは指定の構造でデータを出力します。
...
3. **JSON Schema エディタ**による処理後、LLMから出力される**構造化データ**は以下のようになります。
